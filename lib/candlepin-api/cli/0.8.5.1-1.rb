...
No description available for this entrypoint.
  ...
  CandlepinCLI.subcommand('UpdateLastCheckinConsumer') do |cdef,*args|
    name = args[0]
    cdef.usage(name + ' [OPTIONS]')
    cdef.summary("org.candlepin.resource.ConsumerResource.updateLastCheckin")
    cdef.description(<<-EOD)
No description available for this entrypoint.

    REST: PUT /candlepin/consumers/{consumer_uuid}/checkin
    ...
    cdef.required(nil, :checkin_date,
                  "[Query argument] A string.") do |value,cmd|
      CandlepinCLI.options.checkin_date = value
    end
    cdef.run do |opts,args,cmd2|
      CandlepinCLI.context.entrypoint = eval('CandlepinAPI::UpdateLastCheckinConsumer')
    end
  end                           # entrypoint UpdateLastCheckinConsumer
...
No description available for this entrypoint.
    ...
    REST: GET /candlepin/owners/{owner_key}/statistics/{type}
